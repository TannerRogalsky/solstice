...
const KNOWN_SUBKEYS: &[&str] = &["sem", "normalized"];
    ...
    A: Iterator<Item = &'a Attribute> + Clone,
{
    ...
    match struct_.fields {
        Fields::Unnamed(unnamed_fields) => {
            ...
            let output =
                process_struct(ident, indexed_vertex_attrib_descs, Vec::new(), fields_types);
            Ok(output.into())
        }
    ...
    FN: Into<Option<&'a mut Vec<Ident>>>,
{
    ...
    let normalized =
        get_field_attr_once(&ident, &field.attrs, "vertex", "normalized", KNOWN_SUBKEYS)
    ...
    let vertex_attrib_desc = if normalized {
        quote! { (<#field_ty as graphics::vertex::VertexAttrib>::VERTEX_ATTRIB_DESC).normalize() }
    } else {
        quote! { <#field_ty as graphics::vertex::VertexAttrib>::VERTEX_ATTRIB_DESC }
    };

    let indexed_vertex_attrib_desc_q = quote! {
        graphics::vertex::VertexBufferDesc::new::<#sem_type>(
          <#field_ty as graphics::vertex::HasSemantics>::SEMANTICS,
    //      #instancing,
          #vertex_attrib_desc,
        )
      };
...
fn process_struct(
    ...
    quote! {
      // Vertex impl
      unsafe impl graphics::vertex::VertexNew for #struct_name {
        fn vertex_desc() -> graphics::vertex::VertexDesc {
